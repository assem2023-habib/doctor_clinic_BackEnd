...
class AppointmentControllerDoc
{
    ...
    #[OA\Get(
        ...
        description: 'Patients see own appointments, doctors see own, admins/receptionists see all. Supports multiple statuses and doctors, date/time ranges, and custom ordering.',
        tags: ['Appointments'],
        security: [['bearerAuth' => []]],
        parameters: [
            new OA\Parameter(name: 'limit', in: 'query', schema: new OA\Schema(type: 'integer', default: 20, maximum: 100), description: 'Items per page (max 100)'),
            new OA\Parameter(name: 'page', in: 'query', schema: new OA\Schema(type: 'integer', default: 1), description: 'Page number'),
            new OA\Parameter(name: 'status', in: 'query', schema: new OA\Schema(type: 'array', items: new OA\Items(type: 'string')), style: 'form', explode: false, description: 'Filter by status(es). Supports single (?status=accepted) or multiple (?status[]=set&status[]=accepted)'),
            new OA\Parameter(name: 'date', in: 'query', schema: new OA\Schema(type: 'string', format: 'date'), description: 'Filter by exact date (Y-m-d). When set, from_date/to_date are ignored'),
            new OA\Parameter(name: 'from_date', in: 'query', schema: new OA\Schema(type: 'string', format: 'date'), description: 'Filter from date (Y-m-d)'),
            new OA\Parameter(name: 'to_date', in: 'query', schema: new OA\Schema(type: 'string', format: 'date'), description: 'Filter to date (Y-m-d)'),
            new OA\Parameter(name: 'from_time', in: 'query', schema: new OA\Schema(type: 'string', example: '09:00'), description: 'Filter by start_time >= value (H:i)'),
            new OA\Parameter(name: 'to_time', in: 'query', schema: new OA\Schema(type: 'string', example: '17:00'), description: 'Filter by end_time <= value (H:i)'),
            new OA\Parameter(name: 'doctor_id', in: 'query', schema: new OA\Schema(type: 'array', items: new OA\Items(type: 'string', format: 'uuid')), style: 'form', explode: false, description: 'Filter by doctor(s). Single (?doctor_id=uuid) or multiple (?doctor_id[]=uuid1&doctor_id[]=uuid2). Patients can filter by their own doctors.'),
            new OA\Parameter(name: 'order_by', in: 'query', schema: new OA\Schema(type: 'string', enum: ['created_at', 'appointment_date', 'start_time'], default: 'created_at'), description: 'Sort field'),
            new OA\Parameter(name: 'order_dir', in: 'query', schema: new OA\Schema(type: 'string', enum: ['asc', 'desc'], default: 'desc'), description: 'Sort direction'),
        ],
        responses: [
